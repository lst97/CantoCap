...
export class AppStateService {
  ...
  private logger: MainProcessLogger = MainLogger.createScopedLogger('AppStateService');
  ...
  getState(): AppStateSchema {
    ...
    if (state.activeWorkspaceId === undefined) {
      this.logger.warn(
        '⚠️ AppStateService: Preventing undefined activeWorkspaceId from being returned'
      );
  ...
  validateAndCleanState(workspaceExistsCallback: (id: string) => boolean): void {
    ...
    if (state.activeWorkspaceId && !workspaceExistsCallback(state.activeWorkspaceId)) {
      this.logger.warn('Clearing invalid active workspace:', {
    ...
    const validRecentWorkspaces = state.recentWorkspaces.filter((id) => {
      ...
      if (!exists) {
        this.logger.warn('Removing invalid recent workspace:', {
          recentWorkspaceId: id,
        });
      }
      return exists;
    });
    ...
    if (hasChanges) {
      this.logger.log('App state cleaned up - removed invalid workspace references');
    }
  }
}
