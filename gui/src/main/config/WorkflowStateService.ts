...
export class WorkflowStateService {
  ...
  private logger: MainProcessLogger = MainLogger.createScopedLogger('WorkflowStateService');
  ...
  getWorkflowState(workspaceId: string): {
    currentStep: StepType;
    stepStates: Record<StepType, StepStatusType>;
  } {
  ...
  setCurrentStep(workspaceId: string, step: StepType): void {
    ...
    this.logger.info(`💾 Persisted workflow step for workspace ${workspaceId}: ${step}`);
  }
  ...
  setStepState(workspaceId: string, step: StepType, state: StepStatusType): void {
    ...
    this.logger.info(`💾 Persisted step state for workspace ${workspaceId}: ${step} = ${state}`);
  }
  ...
  resetWorkflowState(workspaceId: string): void {
    ...
    this.store.set('workspaceStates', allStates);
    this.logger.info(`💾 Reset workflow state for workspace ${workspaceId}`);
  }
  ...
  removeWorkspaceState(workspaceId: string): void {
    ...
    this.store.set('workspaceStates', allStates);
    this.logger.info(`💾 Removed workflow state for workspace ${workspaceId}`);
  }
  ...
  cleanupOldStates(activeWorkspaceIds: string[]): void {
    ...
    Object.keys(allStates).forEach((workspaceId) => {
      ...
      if (!activeWorkspaceIds.includes(workspaceId) && state.lastUpdated < cutoffTime) {
        delete allStates[workspaceId];
        hasChanges = true;
        this.logger.info(`🧹 Cleaned up old workflow state for workspace ${workspaceId}`);
      }
    });
  ...
  getAllWorkspaceStates(): Record<
    string,
    { currentStep: StepType; stepStates: Record<StepType, StepStatusType>; lastUpdated: number }
  > {
  ...
  reset(): void {
    this.store.clear();
    this.logger.info('💾 Reset all workflow states');
  }
}
