...
export class DependencyChecker {
  ...
  constructor() {
    this.dependencies = {
      python: {
        ...
        versionPattern: /Python 3\.12\.\d+/,
        downloadUrl: 'https://www.python.org/downloads/',
        critical: true,
        helpText: 'Python 3.12 is required. Consider using pyenv for version management.',
      },
      ffmpeg: {
        name: 'FFmpeg',
        commands: ['ffmpeg'],
  ...
  public async checkAll(
    progressCallback?: (statuses: Record<string, DependencyStatus>) => void
  ): Promise<Record<string, DependencyStatus>> {
    const results: Record<string, DependencyStatus> = {};
    const initialStatuses: Record<string, DependencyStatus> = {};
    ...
    if (progressCallback) {
      ...
      progressCallback(initialStatuses);
    }

    for (const [key, dependency] of Object.entries(this.dependencies)) {
      results[key] = await this.checkDependency(dependency);
      if (progressCallback) {
        const currentStatuses = { ...initialStatuses, ...results };
        progressCallback(currentStatuses);
      }
    }
  ...
  public async checkPythonModules(
    pythonPath: string
  ): Promise<Record<string, { available: boolean; error: string | null }>> {
    // Check if required Python modules are available
    const requiredModules = ['whisper', 'torch', 'numpy', 'scipy'];
  ...
  private checkPythonModule(pythonPath: string, moduleName: string): Promise<void> {
    return new Promise((resolve, reject) => {
      const process = spawn(
        pythonPath,
        ['-c', `import ${moduleName}; print('${moduleName} available')`],
        {
          stdio: ['ignore', 'pipe', 'pipe'],
          shell: true,
        }
  ...
  public getInstallationInstructions(
    ...
    return (
      instructions[dependencyKey]?.[platform] || 'Please install manually from the official website'
    );
  }
}
