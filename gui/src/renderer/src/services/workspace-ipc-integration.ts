...
import { getWorkspaceDatabase } from './workspace-database'
...
export class WorkspaceIPCIntegration {
  ...
  public async syncWorkspaceToMainProcess(workspaceId: string): Promise<{ success: boolean }> {
    ...
    try {
      ...
      const syncResult = await window.cantocapAPI.syncWorkspaceConfig(workspaceId, workspace.config)
  ...
  public async deleteWorkspace(workspaceId: string): Promise<{ success: boolean }> {
    try {
      ...
      if (!mainProcessResult.success) {
        console.warn('Failed to delete workspace from main process, but IndexedDB deletion succeeded')
      }
  ...
  private setupMigrationListeners(): void {
    ...
    const migrationUpdateCleanup = window.cantocapAPI.onWorkspaceMigrationUpdate((status: MigrationStatus) => {
      console.log('Migration status update:', status)
      this.handleMigrationUpdate(status)
    })

    const migrationProgressCleanup = window.cantocapAPI.onWorkspaceMigrationProgress((data: { phase: string, progress: number, message: string }) => {
      console.log('Migration progress:', data)
    })

    const migrationRollbackCleanup = window.cantocapAPI.onWorkspaceMigrationRollback((data: { success: boolean, message: string }) => {
      console.log('Migration rollback:', data)
    })

    this.migrationListeners.push(migrationUpdateCleanup, migrationProgressCleanup, migrationRollbackCleanup)
  }
