...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  // Refs to prevent excessive updates during cleanup operations
  const lastSrcRef = useRef<string | undefined>(undefined);

  // Sync local timeRange with initialRange prop changes
  useEffect(() => {
    // Deep comparison for object equality to prevent infinite loops
    const rangesEqual =
      (!initialRange && !timeRange) ||
      (initialRange &&
        timeRange &&
        initialRange.start === timeRange.start &&
        initialRange.end === timeRange.end) ||
      initialRange === timeRange;

    if (!rangesEqual) {
      logger.info('VideoPlayer: Syncing timeRange with initialRange:', initialRange);
      setTimeRange(initialRange || null);
    }
  }, [initialRange, timeRange, logger]);
  ...
  // Convert local file path to media URL (simplified approach matching Step 4)
  useEffect(() => {
    const convertMediaSource = async () => {
      if (!src) {
        logger.debug('🎬 VideoPlayer: No source provided, clearing media URL');
        setMediaUrl(null);
        setIsLoadingMedia(false);
        return;
      }

      // Skip conversion if src hasn't actually changed
      if (lastSrcRef.current === src) {
        return;
      }

      logger.debug('🎬 VideoPlayer: Starting media conversion', { src });
      lastSrcRef.current = src;

      // Check if it's already a localmedia://, file://, data:// or web URL
      if (
        src.startsWith('localmedia://') ||
        src.startsWith('file://') ||
        src.startsWith('data:') ||
        src.startsWith('http://') ||
      ...
      // Convert local file path to media URL (simplified approach)
      setIsLoadingMedia(true);
      try {
        logger.info('🎬 VideoPlayer: Starting media conversion', {
          src,
          currentMediaUrl: mediaUrl,
        });
        const convertedMediaUrl = await window.cantocapAPI.getMediaUrl(src);
        logger.info('🎬 VideoPlayer: IPC call completed', { convertedMediaUrl, src });
        setMediaUrl(convertedMediaUrl);
        logger.info('🎬 VideoPlayer: Successfully converted file path to media URL', {
          src,
          convertedMediaUrl,
        });
      } catch (error) {
        logger.error('🎬 VideoPlayer: Failed to convert media to URL', {
