...
const categorizeProcessingError = (errorMessage: string): ProcessingErrorType => {
  ...
  if (
    ...
    return {
      code: 'SETUP_ERROR',
      message:
        'FFmpeg is required but not found. Please install FFmpeg to process audio/video files.',
      details: {
        component: 'FFmpeg',
  ...
  if (message.includes('python') || message.includes('engine') || message.includes('subprocess')) {
    return {
      ...
      message:
        'CantoCap engine is not available. Please check your Python installation and engine setup.',
      details: {
        component: 'Engine',
        originalError: errorMessage,
...
const validateProcessingPrerequisites = async (): Promise<{
  isValid: boolean;
...
const attemptAutomaticRecovery = async (
  ...
  logger = createComponentLogger('ProcessingStep')
): Promise<{ recovered: boolean; newError?: ProcessingErrorType }> => {
  logger.debug(`Attempting automatic recovery for error: ${error.code}`);
  ...
  switch (error.code) {
    case 'SETUP_ERROR':
      if (error.details?.component === 'FFmpeg') {
        ...
        if (validation.isValid) {
          logger.debug('FFmpeg recovery successful');
    ...
    case 'IPC_ERROR':
      ...
      logger.debug('Attempting IPC recovery');
    ...
    case 'ENGINE_NOT_FOUND':
      ...
      logger.debug('Attempting engine recovery');
      await new Promise((resolve) => setTimeout(resolve, 3000));
      return { recovered: false }; // Would need engine restart capability

    default:
      logger.debug(`No automatic recovery available for error: ${error.code}`);
...
const handleProcessingError = async (
  ...
  logger.error(`Processing error [${error.code}]: ${error.message}`, { error });

  // Attempt automatic recovery for recoverable errors (max 2 retries)
  if (error.recoverable && retryCount < 2) {
    logger.debug(`Attempting recovery (attempt ${retryCount + 1}/2)`);

    const recoveryLogs = [
      ...localLogs,
      `Error [${error.code}]: ${error.message}`,
      `Attempting automatic recovery (${retryCount + 1}/2)...`,
    ];
  ...
  const errorContext = error.recoverable
    ? ' (Try again or check suggestions)'
  ...
  if (error.recoverable && error.details?.suggestion) {
    logger.debug(`Recovery suggestion: ${error.details.suggestion}`);
  }
};

export const ProcessingStep: React.FC = () => {
  const logger = createComponentLogger('ProcessingStep');
  ...
  useEffect(() => {
    if (processing.status === 'running' || processing.status === 'completed') {
      logger.debug('Resetting retry count due to successful processing state');
      setRetryCount(0);
      setIsRecovering(false);
    }
  }, [processing.status, logger]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (currentState === 'running') {
        ...
        logger.debug(`Timer stopped - status changed to: ${currentState}`);
        clearInterval(interval);
      }
    }, 1000);

    return () => clearInterval(interval);
  }, [processing.status, processing.startTime, updateStepContent, logger]);
    ...
    <>
      <ProcessingErrorBoundary
        ...
        onEngineError={(error, errorInfo) => {
          logger.error('Engine error in ProcessingStep', { error, errorInfo });
          ...
          handleProcessingError(
            ...
            .catch((recoveryError) => {
              logger.error('Error during engine recovery attempt', { error: recoveryError });
              setIsRecovering(false);
            });
        }}
      >
        ...
        >
                  ...
                  {processing.status === 'running' && (localPhase || processing.currentPhase) && (
                    ...
                    >
                      ...
                      >
                        {isRecovering
                          ? 'Recovering...'
                          : localPhase || processing.currentPhase || 'Processing...'}
                      </Typography>
                  ...
                  >
                    <Typography
                      ...
                      sx={{
                        ...
                        color: processing.status === 'completed' ? 'success.main' : 'warning.main',
                      }}
                    >
                      {processing.progress}%
                    </Typography>
          ...
          <Box
            sx={{
              ...
              gap: 3,
              maxWidth:
                processing.status === 'completed' || processing.status === 'error' ? 900 : 600,
              mx: 'auto',
              width: '100%',
            }}
          >
