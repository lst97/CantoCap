...
export const SubtitleEditor: React.FC = () => {
  ...
  const { isSaving, saveError } = useSaveState();
  const {
    updateSubtitle,
    deleteSubtitle,
    splitSubtitle,
  ...
  const { navigateToStep, setStepState } = useWorkflowActions();
  ...
  const isNewSubtitle = editingSubtitle && !editingSubtitle.text && !editingSubtitle.translation;

  // Check if subtitle has translation
  const hasTranslation =
    editingSubtitle && editingSubtitle.translation && editingSubtitle.translation.trim();
  ...
  useEffect(() => {
    if (editingSubtitle) {
      ...
      setShowTranslationField(
        !!(editingSubtitle.translation && editingSubtitle.translation.trim())
      );
    }
  }, [editingSubtitle]);
  ...
  const handleTextChange = (value: string) => {
    setEditText(value);
    setHasChanges(
      editingSubtitle?.text !== value || (editingSubtitle?.text || '') !== editTranslation
    );
  };
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      if (!navigationResult.success) {
        throw new Error(
          `Navigation to export step failed: ${navigationResult.error || 'Unknown error'}`
        );
      }
  ...
  const handleSplit = async () => {
    if (editingSubtitle) {
      const splitTime = currentTime || (editingSubtitle.startTime + editingSubtitle.endTime) / 2;

      if (splitTime > editingSubtitle.startTime && splitTime < editingSubtitle.endTime) {
        try {
  ...
  const handleMergeNext = async () => {
    ...
    const currentIndex = subtitles.findIndex((s) => s.id === editingSubtitle.id);
      ...
      <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <Box sx={{ mt: 'auto', pt: 2 }}>
          <Button
            ...
            startIcon={
              isExporting ? <CircularProgress size={20} color='inherit' /> : <ExportIcon />
            }
            sx={{
              py: 2,
              px: 3,
