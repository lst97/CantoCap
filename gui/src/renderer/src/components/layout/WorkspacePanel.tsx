...
export const WorkspacePanel: React.FC<WorkspacePanelProps> = ({
  ...
  }, [workspaceListRaw, activeWorkspaceId])
  ...
  const [currentWorkspaceForGroup, setCurrentWorkspaceForGroup] = useState<WorkspaceWithGrouping | null>(null)
  ...
  const handleGroupToggle = useCallback(async (groupId: string) => {
    try {
  ...
  const handleWorkspaceContextMenu = useCallback((event: React.MouseEvent<HTMLElement>, workspace: WorkspaceWithGrouping) => {
    ...
    if (target && document.contains(target) && target.offsetParent !== null && target.isConnected) {
  ...
  const handleGroupContextMenu = useCallback((event: React.MouseEvent<HTMLElement>, group: WorkspaceWithGrouping['group']) => {
    event.preventDefault()
    const target = event.currentTarget
    if (target && document.contains(target) && target.offsetParent !== null && target.isConnected && group) {
      // Additional check: ensure the element has proper dimensions
      const rect = target.getBoundingClientRect()
      if (rect.width > 0 && rect.height > 0) {
        setGroupContextMenu({
  ...
  useEffect(() => {
    ...
    if (groupContextMenu && groupContextMenu.anchorEl) {
      if (!document.contains(groupContextMenu.anchorEl) || groupContextMenu.anchorEl.offsetParent === null) {
        setGroupContextMenu(null)
      }
    }
    ...
    try {
      ...
      if (groupInfo && workspaceId) {
        ...
        if (groupInfo.action === 'new' && groupInfo.newGroupName) {
          groupId = await createGroup(groupInfo.newGroupName, groupInfo.newGroupColor || 'default')
        }
  ...
  }, [createWorkspace, duplicateWorkspace, createGroup, addWorkspaceToGroup])
  ...
  const handleRenameWorkspace = useCallback((workspaceId: string) => {
    const workspace = workspaces.find(w => w.id === workspaceId)
    if (workspace) {
  ...
  const handleConfirmRenameWorkspace = useCallback(async (newName: string) => {
  ...
  }, [renameWorkspace, workspaceToRename])
  ...
  const handleCreateNewGroup = useCallback((workspaceId: string) => {
    const workspace = workspaces.find(w => w.id === workspaceId)
    if (workspace) {
            ...
            <Box key={group.id} sx={{ width: '100%', mb: 2.5 }}>
              ...
              <Tooltip title={group.name} placement="right" arrow>
                ...
                >
                  ...
                  >
                    {isExpanded ? <KeyboardArrowDownIcon fontSize="inherit" /> : <KeyboardArrowLeftIcon fontSize="inherit" />}
                  </Box>
                  <Typography
                    variant="caption"
                    sx={{
                ...
                >
                  ...
                  <Box sx={{ position: 'relative' }}>
                    {groupData.workspaces.slice(0, 2).map((workspace, index) => (
                      <Tooltip key={workspace.id} title={`${workspace.name} (${group.name})`} placement="right">
                        <Box
                        ...
                        sx={{
                          ...
                          '&:hover': {
                            ...
                            backgroundColor: workspace.backgroundColor 
                              ? (workspace.isActive ? workspace.backgroundColor : workspace.backgroundColor)
                              : (workspace.isActive ? 'primary.main' : 'primary.dark'),
      ...
      {groupContextMenu && groupContextMenu.group && (
        <GroupContextMenu
          ...
          onRenameGroup={async (groupId: string, newName: string) => {
            ...
            const success = await window.electron.ipcRenderer.invoke('group:update', groupId, { name: newName })
            if (!success) {
          ...
          onChangeGroupColor={async (groupId: string, color) => {
            ...
            const success = await window.electron.ipcRenderer.invoke('group:update', groupId, { color })
            if (!success) {
