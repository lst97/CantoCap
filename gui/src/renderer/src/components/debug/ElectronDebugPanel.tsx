...
export const ElectronDebugPanel: React.FC<DebugPanelProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'performance' | 'ipc' | 'state' | 'system'>('overview')
  ...
  const collectSystemInfo = useCallback(async (): Promise<void> => {
    try {
      ...
      if (envInfo.memory?.available && typeof window !== 'undefined' && 'performance' in window) {
      ...
      setSystemInfo({
        ...
        appVersion: envInfo.electron?.appVersion || 'unknown',
        processId: envInfo.process?.pid || 'unknown',
        memoryUsage: memoryInfo,
        ipcHealth: envInfo.electron?.ipcHealthy || false,
        contextIsolated: envInfo.electron?.contextIsolated || false,
        sandboxed: envInfo.electron?.sandboxed || false
      })

      // Update performance metrics
      if (envInfo.session?.performanceMetrics) {
        setPerformanceMetrics(envInfo.session.performanceMetrics)
      }

      // Update IPC calls and state changes
      if (envInfo.session?.ipcCalls) {
        setIpcCalls(envInfo.session.ipcCalls.slice(-20)) // Last 20 calls
      }

      if (envInfo.session?.stateChanges) {
        setStateChanges(envInfo.session.stateChanges.slice(-20)) // Last 20 changes
      }
  ...
  const openDevTools = useCallback(async (): Promise<void> => {
    try {
      ...
      } else if (window.cantocapAPI) {
        await window.cantocapAPI.openDevTools()
      }
    } catch (error) {
  ...
  const testIPC = useCallback(async (): Promise<void> => {
    try {
      ...
      if (window.electronAPI) {
        ...
        await window.cantocapAPI.getPlatform()
        success = true
      }
  ...
  const getStateColor = useCallback((state: StepState): string => {
    switch (state) {
      case StepState.Ready: return '#2196F3'
      case StepState.Complete: return '#4CAF50'
      case StepState.Error: return '#f44336'
      case StepState.Warning: return '#FF9800'
      case StepState.Blocked: return '#9E9E9E'
      case StepState.Skip: return '#607D8B'
            ...
            {activeTab === 'overview' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  ...
                  <button onClick={openDevTools} style={{
                  ...
                  <button onClick={collectSystemInfo} style={{
                    background: '#9C27B0', border: 'none', color: 'white',
                    padding: '4px 8px', borderRadius: '4px', fontSize: '10px', cursor: 'pointer'
                  }}>
                    Refresh
                  </button>
                  ...
                  ipcCalls.map((call, index) => (
                    <div key={index} style={{
                      ...
                      <div>
                        ...
                        {call.error && <div style={{ fontSize: '10px', color: '#ff6b6b' }}>{call.error}</div>}
                      </div>
            ...
            {activeTab === 'state' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <div style={{ marginBottom: '12px' }}>
                  <div style={{ fontSize: '11px', color: '#ccc', marginBottom: '4px' }}>Current Step</div>
                  <div style={{
                    padding: '6px 10px',
                    background: '#2196F3',
                    borderRadius: '4px',
                ...
                <div style={{ marginBottom: '12px' }}>
                  <div style={{ fontSize: '11px', color: '#ccc', marginBottom: '4px' }}>All Steps</div>
                  {steps.map((step) => (
                    <div key={step.id} style={{
                      ...
                      padding: '4px 8px',
                      margin: '2px 0',
                      background: step.id === currentStepId ? '#444' : '#333',
                      borderRadius: '4px',
                      borderLeft: `3px solid ${getStateColor(step.stateMetadata.state)}`
                    }}>
                      <span>{step.id}</span>
                      <span style={{
                        color: getStateColor(step.stateMetadata.state),
                        fontWeight: 'bold',
                        fontSize: '10px'
                      }}>
                        {step.stateMetadata.state.toUpperCase()}
                      </span>
                    </div>
                  ))}
                </div>
                ...
                <div>
                  <div style={{ fontSize: '11px', color: '#ccc', marginBottom: '4px' }}>Recent State Changes</div>
                  {stateChanges.length === 0 ? (
                    <div style={{ color: '#888', fontStyle: 'italic' }}>No state changes recorded</div>
                  ) : (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                ...
                <div style={{ display: 'grid', gap: '8px' }}>
                  <div style={{ background: '#333', padding: '8px', borderRadius: '4px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: '4px 8px', fontSize: '11px' }}>
                      <span>Platform:</span><span>{systemInfo.platform}</span>
                      <span>App Version:</span><span>{systemInfo.appVersion}</span>
                      <span>Process ID:</span><span>{systemInfo.processId}</span>
                      <span>Context Isolated:</span><span>{systemInfo.contextIsolated ? 'Yes' : 'No'}</span>
                      <span>Sandboxed:</span><span>{systemInfo.sandboxed ? 'Yes' : 'No'}</span>
                  ...
                  <div style={{ background: '#333', padding: '8px', borderRadius: '4px' }}>
                    ...
                    {typeof systemInfo.memoryUsage === 'object' ? (
                      <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: '4px 8px', fontSize: '11px' }}>
                        <span>Used:</span><span>{systemInfo.memoryUsage.used}</span>
                        <span>Total:</span><span>{systemInfo.memoryUsage.total}</span>
                        <span>Limit:</span><span>{systemInfo.memoryUsage.limit}</span>
                      </div>
                    ) : (
                      <div style={{ fontSize: '11px', color: '#888' }}>{systemInfo.memoryUsage}</div>
                    )}
                  </div>
                  ...
                  <div style={{ background: '#333', padding: '8px', borderRadius: '4px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: '4px 8px', fontSize: '11px' }}>
                      ...
                      <span>Refresh Rate:</span><span>{refreshInterval}ms</span>
                      <span>IPC Calls:</span><span>{ipcCalls.length} recorded</span>
                      <span>State Changes:</span><span>{stateChanges.length} recorded</span>
                    </div>
                  </div>
                </div>
              </div>
