...
export interface UseSubtitlePersistenceResult {
  ...
  // Actions
  loadFile: (fileId: string, options?: { useCache?: boolean; validateOnLoad?: boolean }) => Promise<SubtitleFileContent | null>
  saveFile: (fileId: string, content: SubtitleFileContent, options?: { createBackup?: boolean; validate?: boolean }) => Promise<void>
  createFile: (content: SubtitleFileContent, fileType?: 'original' | 'modified') => Promise<string>
 ...
 * Default options
 */
const DEFAULT_OPTIONS: Required<UseSubtitlePersistenceOptions> = {
  autoValidate: true,
  debounceMs: 1000,
  syncEnabled: true,
  mergeStrategy: 'merge',
  ...
  onFileOperationSuccess: undefined
}
...
export function useSubtitlePersistence(
  ...
  const [fileStatus, setFileStatus] = useState<SubtitleFileStatus | null>(null)
  ...
  // Validation state
  const [validationResults, setValidationResults] = useState<Record<string, SubtitleValidationResult>>({})
  ...
  const handleError = useCallback((error: unknown, context?: string) => {
    const subtitleError = isSubtitleFileError(error) 
      ? error 
      : new Error(`${context || 'Operation'} failed: ${error instanceof Error ? error.message : 'Unknown error'}`) as SubtitleFileError
   ...
   */
  const handleOperationSuccess = useCallback((operation: string, result: any) => {
  ...
  const createFile = useCallback(async (
    content: SubtitleFileContent,
    fileType: 'original' | 'modified' = 'modified'
  ): Promise<string> => {
    ...
    try {
      ...
      const fileId = await service.createFile(content, {
        compress: content.subtitles.length > 100, // Compress large files
        fileType: fileType
      })
      
      setCurrentFiles(prev => ({ ...prev, [fileId]: content }))
      handleOperationSuccess('create', { fileId, content, fileType })
   ...
   */
  const validateFile = useCallback(async (fileId: string): Promise<SubtitleValidationResult> => {
    if (!activeWorkspaceId) throw new Error('No active workspace')
   ...
   * Create backup
   */
  const createBackup = useCallback(async (fileId: string, description?: string): Promise<string> => {
    if (!activeWorkspaceId) throw new Error('No active workspace')
   ...
   * Update session data
   */
  const updateSession = useCallback(async (sessionData: Partial<SubtitleSessionData>): Promise<void> => {
    if (!activeWorkspaceId) return
   ...
   * Load session
   */
  const loadSession = useCallback(async (sessionType: SubtitleSessionData['sessionType'] = 'review'): Promise<void> => {
    if (!activeWorkspaceId) return
   ...
   */
  const refreshFile = useCallback(async (fileId: string): Promise<void> => {
    if (!activeWorkspaceId) return
   ...
   * Enable auto-save
   */
  const enableAutoSave = useCallback((interval?: number): void => {
   ...
   * Track file changes
   */
  const markFileDirty = useCallback((fileId: string): void => {
  ...
  useEffect(() => {
    const updateMetrics = async () => {
      try {
        ...
        if (mergedOptions.enablePerformanceMonitoring && service.performanceTracker?.getAnalytics) {
          const analytics = service.performanceTracker.getAnalytics()
