...
class TranscriptionChunk:
    ...
    def from_whisper_chunk(cls, whisper_data: Dict[str, Any]) -> "TranscriptionChunk":
        ...
        timestamp = whisper_data.get("timestamp", [0, 0])
        
        # Validate timestamp array and handle None values
        if not isinstance(timestamp, list) or len(timestamp) < 2:
            timestamp = [0, 0]
        
        start_seconds = max(0.0, timestamp[0] if timestamp[0] is not None else 0.0)
        end_seconds = max(0.0, timestamp[1] if timestamp[1] is not None else 0.0)
        
        # Ensure end time is after start time
        if end_seconds <= start_seconds:
            end_seconds = start_seconds + 0.1  # Minimum 100ms duration
